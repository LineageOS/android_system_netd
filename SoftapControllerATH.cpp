...
static int set_wifi_power(int on) {
    ...
    sleep(3); /* Always sleep 3 seconds to avoid race conditions */
	...
	set_wifi_power(0);
	{
		...
		int fd = -1;
 		usleep(1000000);
    		...
    		fd = open("/sys/module/ar6000/parameters/ifname", O_WRONLY);
    		if (fd < 0) {
        		LOGE("failed to change interface name");
        		ret = -1;
		}
    		if (write(fd, "athap0", 6) < 0) {
        		LOGE("failed to change interface name");
        		ret = -2;
		}
		close(fd);
    		sleep(7); /* Give it a long while after a name change... */
...
int SoftapController::startSoftap() {
   ...
   if ((pid = fork()) < 0) {
    ...
    if (!pid) {
        ...
        sleep(2); /* Give the driver time to settle... */
        ifc_init();
        ifc_up("athap0");
