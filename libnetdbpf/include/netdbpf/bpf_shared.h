...
// Maximum number of input interface deny rules (per uid).
#define UID_MAX_IF_DROP 3

typedef struct {
    // Allowed interface index. Only applicable if IIF_MATCH is set in the rule bitmask above.
    uint32_t iif;
    // Denied in/out interface indexes. Only applicable if IF_MATCH_DROP is set in the
    // rule bitmask above.
    uint32_t if_drop[UID_MAX_IF_DROP];
