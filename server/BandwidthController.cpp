...
const std::array<std::string, UID_MAX_IF_DROP> APP_RESTRICT_USE_CASES =
        { "data", "vpn", "wlan" };
...
int BandwidthController::manipulateRestrictAppsInOut(const std::string& usecase,
    ...
    if (!mBpfSupported) {
        ...
        for (const auto& appStrUid : appStrUids) {
            ...
            ALOGE("samm: uid = %i", uid);
            // map use case to drop interface slot
            int slot = -1;
            ...
            if (op == IptOpInsert) {
                status = gCtls->trafficCtrl.addUidInterfaceDropRules(slot,
                    android::net::RouteController::getIfIndex(iface.c_str()), uids);
            } else {
                status = gCtls->trafficCtrl.removeUidInterfaceDropRules(slot, uids);
            }
            if (!isOk(status)) {
