...
binder::Status NetdNativeService::bandwidthAddRestrictAppsOnInterface(const std::string& usecase, const std::string& ifName, int32_t uid) {
    NETD_LOCKING_RPC(gCtls->bandwidthCtrl.lock, PERM_NETWORK_STACK, PERM_MAINLINE_NETWORK_STACK);
    std::vector<std::string> appStrUids = {std::to_string(abs(uid))};
...
binder::Status NetdNativeService::bandwidthRemoveRestrictAppsOnInterface(const std::string& usecase, const std::string& ifName, int32_t uid) {
    NETD_LOCKING_RPC(gCtls->bandwidthCtrl.lock, PERM_NETWORK_STACK, PERM_MAINLINE_NETWORK_STACK);
    std::vector<std::string> appStrUids = {std::to_string(abs(uid))};
