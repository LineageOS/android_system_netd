...
Status TrafficController::addUidInterfaceDropRules(const int if_blacklist_slot, const int iface,
                                                   const std::vector<int32_t>& uidsToAdd) {
    if (mBpfLevel == BpfLevel::NONE) {
        ALOGW("UID ingress interface filtering not possible without BPF owner match");
        return statusFromErrno(EOPNOTSUPP, "eBPF not supported");
    ...
    for (auto uid : uidsToAdd) {
        ...
        if (!isOk(result)) {
            ALOGW("addRule failed(%d): uid=%d iface=%d if_blacklist_slot=%d (for addUidInterfaceDropRules)",
                  result.code(), uid, iface, if_blacklist_slot);
        } else {
            ALOGW("addRule success(%d): uid=%d iface=%d if_blacklist_slot=%d (for addUidInterfaceDropRules)",
                  result.code(), uid, iface, if_blacklist_slot);
        }
    }
    return netdutils::status::ok;
...
Status TrafficController::removeUidInterfaceDropRules(const int if_blacklist_slot,
    ...
    for (auto uid : uidsToDelete) {
        ...
        if (!isOk(result)) {
            ALOGW("removeRule failed(%d): uid=%d if_blacklist_slot=%d (for removeUidInterfaceDropRules)",
                  result.code(), uid, if_blacklist_slot);
        } else {
            ALOGW("removeRule success(%d): uid=%d if_blacklist_slot=%d (for removeUidInterfaceDropRules)",
                  result.code(), uid, if_blacklist_slot);
        }
    }
    return netdutils::status::ok;
