...
Status TrafficController::addUidInterfaceBlacklist(const int ifBlacklistSlot, const int iface,
    ...
    for (const auto& appStrUid : appStrUids) {
        ...
        if (!isOk(result)) {
            ALOGW("addRule failed(%d): uid=%d iface=%d ifBlacklistSlot=%d (for addUidInterfaceBlacklist)",
                  result.code(), uid, iface, ifBlacklistSlot);
        }
    }
    return netdutils::status::ok;
}
...
Status TrafficController::removeUidInterfaceBlacklist(const int ifBlacklistSlot,
    ...
    for (const auto& appStrUid : appStrUids) {
        ...
        if (!isOk(result)) {
            ALOGW("removeRule failed(%d): uid=%d ifBlacklistSlot=%d (for removeUidInterfaceBlacklist)",
